    ...
    public class GetCalendarTests
    {
        ...
        public async Task GivenCompleteRequest_WhenGetCalendar_ThenAGetReturnsTheCalculatedCalendar()
        {
            ...
            using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start=2000-01-01&nights=5"))
            {
                ...
                AssertResult(postRentalResult, postBooking1Result, postBooking2Result, getCalendarResult);
            }
        }

        private static void AssertResult(ResourceIdViewModel postRentalResult, ResourceIdViewModel postBooking1Result, ResourceIdViewModel postBooking2Result, CalendarViewModel getCalendarResult)
        {
        ...
        private async Task<ResourceIdViewModel> PostBookingRequest(BookingBindingModel postBooking1Request)
        {
            using (var postBooking1Response = await _client.PostAsJsonAsync($"/api/v1/bookings", postBooking1Request))
            {
                ...
                return await postBooking1Response.Content.ReadAsAsync<ResourceIdViewModel>();
            }
        }

        private async Task<ResourceIdViewModel> CreateRequest(RentalBindingModel postRentalRequest)
        {
